...
* \brief 
*/
void filterRMAF(float32_t* inputArr, float32_t* outputArr, uint32_t signalLen, uint32_t filterPts){
	uint32_t i,j;
	float64_t accum = 0;
	for(i=0;i<(filterPts-1);++i){
		accum +=  inputArr[i];
	}
	outputArr[((filterPts-1)/2)] = accum / filterPts;
	for(i=(ceil(filterPts/2));i<((signalLen - (filterPts/2))-1);++i){
		accum +=  inputArr[i+((filterPts-1)/2)] - inputArr[i-((uint32_t)ceil(filterPts/2))];
		outputArr[i] = accum / filterPts;
	}
...
int main(void){
	...
	/*RMAF*/
//	filterRMAF(inputSignal_f32_1kHz_15kHz, OutputSignal_f32, SIGNAL_SIZE, 8);
	filterRMAF(inputSignal_f32_1kHz_15kHz, OutputSignal_f32, SIGNAL_SIZE, 16);
//	filterRMAF(inputSignal_f32_1kHz_15kHz, OutputSignal_f32, SIGNAL_SIZE, 32);
//	filterRMAF(inputSignal_f32_1kHz_15kHz, OutputSignal_f32, SIGNAL_SIZE, 64);
